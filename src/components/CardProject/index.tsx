...
export default function CardProject({name,description,id,language, homepage, index}: cardItem,) {
	...
	return (
		<div className={`w-[90%] m-auto mt-10 [&>div>button]:hover:bg-primary hover:bg-highlights [&>div>button]:hover:text-highlights bg-secondary overflow-hidden rounded-lg border-none [&>object]:hover:scale-110 duration-500 ${index > listLimit && "hidden"}`}>
			<object id={String(id)} className='w-[110%] duration-500' data={`https://raw.githubusercontent.com/EversonDias/${name}/main/readme/cardProject/main.png`} type='image/png'>
						<img
						id={String(id)}
					...
					<div className="flex flex-col items-end bg-secondary w-[90%] p-4 rounded-md">
						...
						<div className="w-full">
							<div className="flex justify-center">
								<p className="text-2xl font-bold hoverUnderline mb-2 duration-500">Linguagem</p>
							</div>
							...
							<p className="text-2xl font-bold hoverUnderline mb-2 mt-4 duration-500">Descrição</p>
							<p className="text-xl w-full">{description}</p>
						</div>
						<div className="flex w-full justify-evenly mt-4">
						<a className='flex gap-2 hover:text-highlights text-xl duration-500' target='_blank' href={`https://github.com/EversonDias/${name}/tree/main#readme`} rel='noreferrer'>Repositório <FiExternalLink/></a>
								{
									homepage && (
										<a className='flex gap-2 hover:text-highlights text-xl duration-500' href={homepage} target='_blank' rel='noreferrer'>Site demo <FiExternalLink/></a>
									)
								}
						</div>
					</div>
